...
public class FileSearch {
    ...
    public File[] listFiles(String directoryPath, String fileNamePattern, int type, boolean isRecursive, int period) {
        ...
        return listFiles(dir, fileNamePattern, type, isRecursive, period);
    }
    @SuppressWarnings("unchecked")
    public File[] listFiles(File dir, String fileNamePattern, int type, boolean isRecursive, int period) {
        if (!dir.exists()) {
        ...
        if (fileNamePattern != null && fileNamePattern.equals("*") && (type != TYPE_FILE)) {
            set.add(dir);
        }
     ...
     *                TYPE_DIR・・・ディレクトリ
     * @param isRecursive 再帰的に検索する場合はtrue
     * @param start    検象開始日時  0:で未設定
     * @param end      検象終了日時  0:で未設定
     * @return 検索にマッチしたファイルオブジェクトリスト
     */
    @SuppressWarnings("unchecked")
    public File[] listFiles(String directoryPath, String fileNamePattern, int type, boolean isRecursive, long start, long end) {
        File dir = new File(directoryPath);
        return listFiles(dir, fileNamePattern, type, isRecursive, start, end);
    }
    @SuppressWarnings("unchecked")
    public File[] listFiles(File dir, String fileNamePattern, int type, boolean isRecursive, long start, long end) {
        ...
        for (int i = 0; i < files.length; i++) {
            File file = files[i];
            addFile(type, fileNamePattern, set, file, start, end);
            // 再帰的に検索＆ディレクトリならば再帰的にリストに追加
            if (isRecursive && file.isDirectory()) {
                listFiles(file.getAbsolutePath(), fileNamePattern, type, isRecursive, start, end);
            }
        }
        return (File[]) set.toArray(new File[set.size()]);
    }
    ...
    private void addFile(int type, String match, TreeSet set, File file,int period) {
        ...
        if (match != null && !match.equals("*") && !file.getName().matches(match)) {
            return;
        }
    ...
    @SuppressWarnings("unchecked")
    private void addFile(int type, String match, TreeSet set, File file, long start, long end) {
        switch (type) {
        case TYPE_FILE:
            if (!file.isFile()) return;
            break;
        ...
        if (start > 0 && file.lastModified() < start) return;
        if (end   > 0 && file.lastModified() > end) return;
        // 全ての条件に該当する場合リストに格納
        set.add(file);
    }

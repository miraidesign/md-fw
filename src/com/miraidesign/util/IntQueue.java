...
//          update 2014-02-18   ishiokatoru@miraidesign.com
//------------------------------------------------------------------------
//
//------------------------------------------------------------------------
...
public  class IntQueue extends Queue {
    ...
    public IntQueue(int size,int appendMode) {
        this(size);
        setAppendMode(appendMode);
    }

    public IntQueue(DataInput in) {
        this(DEFAULT_SIZE);
        try {
            readObject(in);
        } catch (Exception ex) {
    ...
    //--------------------------------
    public IntQueue copy(IntQueue from) {
        clear();
    ...
    public IntQueue copy(Queue from) {
        clear();
        for (int i = 0; i < from.size(); i++) {
            enqueue(((IntQueue)from).peek(i));
        }
        return this;
    }
    //--------------------------------
    ...
    /** 要素の挿入 */
    public synchronized boolean insert(int index, int value) { 
        ...
        if (head < tail) {
            ...
            if (index < tail) {
                ...
                System.out.println("IntQueue insert index error");
            }
        }
        array[index] = value;
    ...
    /** 要素の取り出し */
    public synchronized int dequeue() throws InterruptedException {  // 取り出し
        while (len <= 0) { 
            if (waitMode) wait();
            else         return 0;
        }
    ...
    public synchronized boolean remove(int index) 
        ...
        if (head < tail) {
            ...
            if (index < tail) {
                ...
                System.out.println("IntQueue remove index error");
            }
    ...
    /** 指定個所を覗く */
    public synchronized int peek(int index) {
    ...
    public synchronized boolean poke(int index,int i) {
        if (len <= 0 || index < 0 || index >= len) {
            ...
            array[(head+index)%array.length] = i;
            return true;
        }
    }
    ...
    */
    ...
    /** 内容が同じか？ */
    public boolean equals(IntQueue queue) {
    ...
    /** 
        ...
        先頭、最後の {} [] 等があれば削除する
    */
    public IntQueue addFromCSV(String str) {
    ...
    /** 
        ...
        先頭、最後の {} [] 等があれば削除する
    */
    public IntQueue addFromCSV(CharArray ch) {
        do {
            ...
            for (int i = 0; i < token.size(); i++) {
                enqueue(CharArray.getInt(token.get(i)));
            } 

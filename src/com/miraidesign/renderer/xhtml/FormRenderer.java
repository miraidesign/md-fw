...
public class FormRenderer /*implements ItemRenderer*/ {
    ...
    */
    ...
    static private CharArray drawHidden(CharArray ch, FormData formData,
        ...
        CharArray _site_char = map.getSiteString(_site);
        if ((formData.getInitialMode() && _site_char == null) // サイトマッピングが定義されている時は必要ない
            || mm.ini.getBoolean("["+mm.sectionBase+"]","UseSiteCode")) {
            if (_site > 0) {
                ch.add("    <input type=\"hidden\" name=\"");
